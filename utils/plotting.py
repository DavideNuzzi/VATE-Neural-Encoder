...
def training_summary_plot(model, dataset, losses, label_to_show=None, title_args=dict()):
    ...
    # Training encoding
    z_encoded_train = model.encoder(dataset.neural.data.unsqueeze(0))
    if isinstance(z_encoded_train, tuple):
        z_encoded_train = z_encoded_train[1]
...
def training_summary_plot_iw(model, dataset, losses, label_to_show=None, title_args=dict()):
